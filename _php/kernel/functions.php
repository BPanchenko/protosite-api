...
function array_parse_int(array $arr = []) {
  return array_combine(array_keys($arr), array_map(function($val){
    return (is_numeric($val) && $val < PHP_INT_MAX) ? (int)$val : $val;
  }, $arr));
}

function deleteAllCookies() {
  $domain = preg_replace('/^(?:.*\.)([\w-]+\.[a-z]{2,6})$/i', '$1', $_SERVER['SERVER_NAME']);
  $time = time() - 3600;
  foreach ($_COOKIE as $key=>$value) {
    setCookie($key, '', $time);
    setCookie($key, '', $time, '/');
    setCookie($key, '', $time, '/', $domain);
  }
  return true;
}
...
function getQueryWithoutParameter($param='') {
  ...
  foreach($temp as $phrase) {
    if(trim($phrase) && strpos($phrase, $param . '=') === false) array_push($params, $phrase);
  }
