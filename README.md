...
<p>
	...
	<dl>
		...
		<dt><q>excluded_fields</q></dt>
			<dd>
                ...
                Фреймворк заполняет параметр значением по умолчанию, при переопределении вы увидите неожиданные поля.
                В общем случае исключаемыми из результатов выборки атрибутами, являются: `id` и `is_del`.
            </dd>
		<dt><q>where</q></dt>
			<dd>
				<p>
					Условие выборки из источника данных. Логические выражения перечисляются через запятую (оператор 'AND').<br>
					В качестве первого аргумента логического выражения указывается название параметра запрашиваемой сущности.<br>
					Второй аргумент является значением параметра поиска.<br>
					Аргументы логического выражения разделяются символом ':'(равно), '::'(строгое соответствие) или '!:'(не равно).
				<p>
				<blockquote>
					Фреймворк проверяет наличие полей в источнике данных сущности и для несуществующих
					параметров будет выброшенно исключение, API вернёт 500-ю ошибку с соответствующим описанием.
					<br><br>
					Использование знака равенства для строкового поля источника данных
					подразумевает его замену на опреатор ' LIKE ' в выражении условия поиска,
					а значение параметра будет обрамлено знаком '%' в шаблоне для поиска по вхождению строки в значение поля результатов выборки.
					Для поиска по точному совпадению значения поля с запросом необходимо использовать знак '::'.
				</blockquote>
			</dd>
	...
	<h3>Формат ответа</h3>
	<p>
	    Результат запроса всегда представляет собой объект.
	    Запрашиваемые даннные передаются в атрибуте <code>data</code>.
	    <br>
	    В атрибуте <code>meta</code> передаются метаданные ответа API. Например в случае ошибочного запроса,
	    подробное описание ошибки будет передано в метаданных.
	    <br>
	    Ответ можно дополнять чем угодно, например данными постраничной навигации.
	</p>
	<p>
