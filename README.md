...
<p>
	...
	<dl>
		...
		<dt><q>fallback</q></dt>
			<dd>
            	Фреймворк переключается в режим возврата статических данных из JSON-файлов, путь к дирректории с файлами определяется константой <code>STATIC_DIR</code>. Название файла составляется из элементов конечной точки разделённых символом нижнего подчёркивания <q>_</q>. В случае если файл не был найден возвращается 404-я ошибка с описанием проблемы.<br>
				Необходимо передавать значение <q>true<q>.
            </dd>
		<dt><q>fields</q></dt>
			<dd>
            	Перечень запрашиваемых атрибутов, переданных через запятую. Перестает влиять на результат запроса при указании параметра <code>bulk</code>.<br>
                По умолчанию результат запроса содержит все поля источника данных, которые не упоминаются в <code>excluded_fields</code>.
            </dd>
		<dt><q>excluded_fields</q></dt>
			<dd>
                ...
                В общем случае исключаемыми из результатов выборки атрибутами, являются: <code>id</code> и <code>is_del</code>.
            </dd>
		<dt><q>where</q></dt>
			<dd>
				<p>
					Условие выборки из источника данных. Логические выражения перечисляются через запятую (оператор <q>AND</q>).<br>
					...
					Аргументы логического выражения разделяются символом <q>:</q>(равно), <q>::</q>(строгое соответствие) или <q>!:</q>(не равно).
				<p>
				<blockquote>
					...
					подразумевает его замену на опреатор <q> LIKE </q> в выражении условия поиска,
					а значение параметра будет обрамлено знаком <q>%</q> в шаблоне для поиска по вхождению строки в значение поля результатов выборки.
