		...
		<dt><q>where</q></dt>
			<dd>
				<p>
					...
					В качестве первого аргумента логического выражения указывается название поля источника данных сущности.<br>
					Второй аргумент является значением для поиска по полю источника данных.<br>
					Аргументы логического выражения разделены набором символов, подчиняющихся правилу <q>/:[<>===]{0,1}:/</q>.
				<p>
				<blockquote>
					Фреймворк проверяет наличие полей в источнике данных сущности, которые были переданы в выражениях параметра.<br>
					Использование знака равенства для строкового поля источника данных подразумевает его замену на выражение ' LIKE ' в итоговом запросе выборки, значение для поиска обрамляется знаками '%', т.е. выполняется поиск по совпадению в поле источника данных. Для поиска точного совпадения необходимо использовать знак '=='.
				</blockquote>
			</dd>
