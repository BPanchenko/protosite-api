...
<p>
	...
	<dl>
		...
		<dt><q>fallback</q></dt>
			<dd>
            	Фреймворк переключается в режим возврата статических данных из JSON-файлов дирректории `static`, путь к которой определяется константой `STATIC_DIR`. Название файла составляется из элементов конечной точки разделённых символом нижнего подчёркивания `_`. В случае если файл не был найден возвращается 404-я ошибка с описанием проблемы.
            </dd>
		<dt><q>fields</q></dt>
			<dd>
